...
def render_relations_tab(project_id, persona):
    st.header("🔗 Relations")
    st.caption("Quan hệ giữa các thực thể trong Bible. Chỉnh sửa hoặc xóa từng mục.")
    ...
    def _relations_list_fresh():
        try:
            ...
            _bible = get_bible_list_cached(project_id, st.session_state.get("update_trigger", 0))
            _id_to_name = {e["id"]: e.get("entity_name", "") for e in _bible}
            try:
            ...
            for r in all_rels:
                ...
                src_name = _id_to_name.get(src_id, "?")
                tgt_name = _id_to_name.get(tgt_id, "?")
                ...
                with st.expander(title, expanded=editing):
                    if editing and can_write:
                        ...
                        with col_save:
                            if st.button("💾 Lưu", key=f"rel_save_{rel_id}"):
                                try:
                                    _supabase.table("entity_relations").update({
                    ...
                    if can_delete:
                        if st.button("🗑️ Xóa", key=f"rel_del_{rel_id}"):
                            try:
                                _supabase.table("entity_relations").delete().eq("id", rel_id).execute()
                                invalidate_cache_and_rerun()
                            except Exception as ex:
